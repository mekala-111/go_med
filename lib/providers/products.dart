...
class ProductProvider extends StateNotifier<ProductModel> {
    ...
    try {
      ...
      if (image != null) {
        for (var image in image) {
          request.files.add(await http.MultipartFile.fromPath(
            'image', // Ensure this matches the expected field name
            image.path,
          ));
        }
      }
  ...
  throw Exception("No product found with the selected name: $selectedProduct");
    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        print("sparepart added successfully!");
        getProducts(); // Refresh product list
      } else {
        final errorBody = jsonDecode(response.body);
        final errorMessage =
            errorBody['message'] ?? 'Unexpected error occurred.';
        throw Exception("Error adding sparepart: $errorMessage");
      }
    } catch (error) {
      print("Failed to add sparepart: $error");
      rethrow;
    } finally {
      loadingState.state = false;
    }
